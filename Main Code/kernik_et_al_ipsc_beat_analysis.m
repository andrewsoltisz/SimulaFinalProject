function [AP_measures, flag] = kernik_et_al_ipsc_beat_analysis(time, values, sim_inputs, SA_par)
...
if length(time) > 1000
    if sim_inputs(1) == 0 && max(Vm(500:end))-min(Vm(500:end)) < 5 && min(Vm(500:end)) < -20 % Non beating, and rest (less than 5mV change over time course), and rest is less than -20mV
        ...
        [time1, values1] = ode15s(@kernik_et_al_ipsc_function, [0, 60e3], values(end,:), options, sim_inputs, SA_par); % Run with beating for 60 s, pull ICs after last beat
...
elseif max(Vm(500:end))-min(Vm(500:end)) < 5 && flag(1) == 0 % Non-functional, non-beating cells wiht rest voltage >-20mV
    ...
    if flag(1)~=1 && abs(1-(Ca_i_max_start-Ca_i_min_start)/(Ca_i_max-Ca_i_min))>.02 || abs(1-(Ca_SR_max_start-Ca_SR_min_start)/(Ca_SR_max-Ca_SR_min))>.02% || abs(1-(Na_i_max_start-Na_i_min_start)/(Na_i_max-Na_i_min))>.02
        %if not at SS during run (amplitude of Cai, Nai, or Ca_SR changes more than 2% over run
        flag(4) = 1;
    end
    ...
    for i = 2:length(time)-1
        if Vm(i)>Vm(i-1) && Vm(i)>Vm(i+1) && peak_find==0 && count_peak<=5  % Find peak AP by finding max value after an MDP (goes from positive to negative slope)
            if Vm(i) >- 20 % "real" peaks should have voltage >= -20 mV
                if count_peak <= 0 % if oscillating is happening, and causes counter to get below zero, set flag for oscillating
                    flag(6) = 1;
                    break
                end
                ...
                if count_min > 1 && (count_peak-2) > 0
                    if time(i)-100 < time_peak(count_peak-2) && Vm(i)+20 > mdp(count_min-1) % if peak is found within 20 mV and 100 ms of previous peak
            ...
            elseif sum(peak)>0 % if peak is less than -20 mV, ignore previous minima, find new min (hyperpolarization of AP)
                count_min = count_min-1;
                peak_find = 1;
            end
        ...
        if Vm(i)<Vm(i-1) && Vm(i)<Vm(i+1) && peak_find==1 && count_min<=5  % Find MDP by min value after a peak
            if count_min <= 0 % if oscillating is happening, and causes counter to get below zero, set flag for oscillating
                flag(6) = 1;
                break
            end
            mdp(count_min) = Vm(i);
    ...
    for i = 2:length(time)-1
        ...
        if count <= length(peak_loc(peak_loc~=0)) && count <= length(mdp_loc(mdp_loc~=0)) % APD measurements must be between a found peak and MDP poitn
            ...
            APD_end20 = peak(count)-.2*(amplitude(count));
            ...
            if i>peak_loc(count) && i < mdp_loc(count) && Vm(i)>APD_end20   %Find Vm of 20% amplitude between a peak and MDP during repolarization
                APD20(count) = time(i);
            end
    ...
    if length(APD90(APD90~=0)) > size_APDs
        ...
        APD20=APD20(2: size_APDs+1)-time_dvmax(1: size_APDs);
    ...
    elseif sum(flag(2:6)) ~= 0
        ...
        AP_measures = [BPM, cycle_length, dv_dt_max(1), -1*mdp(1), amplitude(1), plateau(1), APD20(1), APD50(1), APD90(1), Na_i_min, diastolic_ca_avg, Ca_amplitude, Ca_T2Peak, Ca_TransDur, Ca_tau_decay];
