function [parameters, varargout] = base_model_init_parameters_guinea_pig()
% % Default parameter values for the guinea pig base model from:
  ...
  % --- I_Na ---
  parameters(1) = 0.90051970386576; % g_Na;
  parameters(2) = 1; % lambda_Na;

  % --- I_NaL ---
  parameters(3) = 0.03268664681324; % g_NaL;
  ...
  parameters(8) = 1.61849731950290; % g_NaK;

  % --- I_Ks ---
  parameters(9) = 1; % epi;
  parameters(10) = 0.69425481886797; % g_Ks;
  parameters(11) = 0.018; % pNaK;

  % --- I_Kr ---
  parameters(12) = 0.025; % L0;
  parameters(13) = 2.3; % Q;
  parameters(14) = 0.06441088723512; % g_Kr;
  parameters(15) = 1; % lambda_K;

  % --- I_to ---
  parameters(16) = 0.00030623790843; % g_to;

  % --- I_K1 ---
  parameters(17) = 0.85445092339839; % g_K1;

  % --- I_bCl ---
  parameters(18) = 0.00111432748617; % g_bCl;

  % --- I_CaL ---
  parameters(19) = 1.8; % Q10CaL;
  parameters(20) = 0.21193226757346; % g_CaL;
  ...
  parameters(27) = 15.85932177790722; % g_NaCa;
  ...
  parameters(32) = 0.59883878852388; % g_pCa;

  % --- I_bCa ---
  parameters(33) = 0.00051776560756; % g_bCa;
  ...
  parameters(48) = 0.917; % Vc; % cytosol volume
