function [parameters, varargout] = base_model_init_parameters_dog()
% % Default parameter values for the canine base model from:
  ...
  % --- I_Na ---
  parameters(1) = 1.44273596467945; % g_Na;
  parameters(2) = 1; % lambda_Na;

  % --- I_NaL ---
  parameters(3) = 0.00418631181135; % g_NaL;
  ...
  parameters(8) = 1.44222113798785; % g_NaK;

  % --- I_Ks ---
  parameters(9) = 1; % epi;
  parameters(10) = 0.10167009505198; % g_Ks;
  parameters(11) = 0.018; % pNaK;

  % --- I_Kr ---
  parameters(12) = 0.025; % L0;
  parameters(13) = 2.3; % Q;
  parameters(14) = 0.01558060089717; % g_Kr;
  parameters(15) = 1; % lambda_K;

  % --- I_to ---
  parameters(16) = 0.30593159814981; % g_to;

  % --- I_K1 ---
  parameters(17) = 0.51935786273714; % g_K1;

  % --- I_bCl ---
  parameters(18) = 0.00013274332331; % g_bCl;

  % --- I_CaL ---
  parameters(19) = 1.8; % Q10CaL;
  parameters(20) = 0.07697219236346; % g_CaL;
  ...
  parameters(27) = 5.94329406254397; % g_NaCa;
  ...
  parameters(32) = 0.58424022347054; % g_pCa;

  % --- I_bCa ---
  parameters(33) = 0.00065302766612; % g_bCa;
  ...
  parameters(48) = 0.917; % Vc; % cytosolic volume
  parameters(49) = 0.001; % Vd;
  parameters(50) = 0.05; % Vn; % network sarcoplasmic reticulum volume
  parameters(51) = 0.004; % Vs; % junctional sarcoplasmic reticulum volume
  ...
  if nargout == 2
    ...
    parameter_names{50} = 'vnsr';
    parameter_names{51} = 'vjsr';
