function [values] = base_model_rhs_zebrafish(t, states, parameters)
  % Compute the right hand side of the zebrafish base model from:
  ...
  % Assign states
  if length(states)~=26
    error('Expected the states array to be of size 26.');
  end
  m=states(1); j=states(2); mL=states(3); hL=states(4); Xr1=states(5);...
    ...
    bsl=states(23); V_m=states(24); dCaT=states(25); fCaT=states(26);
  ...
  values(25) = (dcat_inf-dCaT)/tau_dcat;
  fcat_inf= 1./(1+exp((V_m +61.7)./5.6));
  tau_fcat=1./(.0153*exp(-(V_m+61.7)./83.3)+ 0.015*exp((V_m+61.7)./15.38));
  values(26) = (fcat_inf-fCaT)/tau_fcat;
  I_CaT= g_CaT*(V_m-eca_sl)*dCaT*fCaT;
